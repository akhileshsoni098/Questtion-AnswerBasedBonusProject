...
const awsLink = async (req, res, next) => {
    try {
        let questionImage = req.files;
        
        if (questionImage) {
            if (Object.keys(questionImage).length == 0) return res.status(400).send({ status: false, message: "Please upload Profile Image" });
            let image = await uploadFile(questionImage[0]);
            req.image = image;
            next()
        }
        else {
